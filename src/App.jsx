...
const TableSection = ({ titre, color, items, setItems, seuilMult }) => {
            ...
            {items.map((it, i) => {
              ...
              return (
                <tr key={it.ref} style={{ background:bg, borderBottom:"1px solid #161b22" }}
                  ...
                  <td style={{ padding:"6px 12px", textAlign:"center" }}><Num value={it.phys} onChange={v=>upd(i,"phys",v)} /></td>
                  <td style={{ padding:"6px 12px", textAlign:"center" }}><Num value={it.ebp}  onChange={v=>upd(i,"ebp",v)} /></td>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#010409", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Space Grotesk',sans-serif" }}>
      ...
      <div style={{ background:"#0d1117", border:"1px solid #21262d", borderRadius:12, padding:"32px 28px", width:360 }}>
        ...
        <button onClick={login} disabled={loading} style={{ width:"100%", padding:11, background:"#f0a500", border:"none", borderRadius:8, color:"#010409", fontSize:14, fontWeight:700, cursor:loading?"not-allowed":"pointer", opacity:loading?.7:1, fontFamily:"'Space Grotesk',sans-serif", marginTop:4 }}>
...
export default function App() {
  ...
  const tabs = [{id:"batteries",label:"Batteries",color:"#f0a500",n:batteries.length},{id:"pneus",label:"Pneus",color:"#4fc3f7",n:pneus.length},{id:"huiles",label:"Huiles",color:"#a78bfa",n:huiles.length}];
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#010409", fontFamily:"'Space Grotesk',sans-serif", color:"#e6edf3" }}>
      ...
      <div style={{ borderBottom:"1px solid #21262d", padding:"0 32px", background:"#0d1117" }}>
        <div style={{ maxWidth:1200, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:60 }}>
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            ...
            <div><div style={{ fontSize:15, fontWeight:600 }}>Meca-V</div><div style={{ fontSize:11, color:"#8b949e", fontFamily:"'DM Mono',monospace" }}>Suivi stock consommables</div></div>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            {lastSaved && <span style={{ fontSize:11, color:"#555", fontFamily:"'DM Mono',monospace" }}>Dernier relevé : {lastSaved}</span>}
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"28px 32px" }}>
        <Alertes batteries={batteries} pneus={pneus} huiles={huiles} />

        <div style={{ display:"flex", gap:4, marginBottom:28, borderBottom:"1px solid #21262d" }}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setOnglet(t.id)} style={{ background:"transparent", border:"none", borderBottom:onglet===t.id?`2px solid ${t.color}`:"2px solid transparent", color:onglet===t.id?t.color:"#8b949e", padding:"10px 20px", cursor:"pointer", fontSize:13, fontWeight:600, fontFamily:"'Space Grotesk',sans-serif", display:"flex", alignItems:"center", gap:8, marginBottom:-1 }}>
              {t.label}
              <span style={{ background:onglet===t.id?t.color:"#21262d", color:onglet===t.id?"#010409":"#8b949e", borderRadius:10, fontSize:10, fontWeight:700, padding:"1px 7px", fontFamily:"'DM Mono',monospace" }}>{t.n}</span>
            </button>
          ))}
        </div>

        {onglet==="batteries" && <TableSection titre="Batteries" color="#f0a500" items={batteries} setItems={setBatteries} seuilMult={2} />}
        {onglet==="pneus"     && <TableSection titre="Pneus — Top 15" color="#4fc3f7" items={pneus} setItems={setPneus} seuilMult={2} />}
        {onglet==="huiles"    && <><Jaugeage diam={diam} setDiam={setDiam} hauteurs={hauteurs} setHauteurs={setHauteurs} /><TableSection titre="Huiles moteur" color="#a78bfa" items={huiles} setItems={setHuiles} seuilMult={3} /></>}
